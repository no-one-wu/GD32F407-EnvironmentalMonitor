...
void OLED_Printf(u8 x,u8 y,u8 size1,u8 mode,const char *fmt,...)
{
    ...
    va_list valist;
    va_start(valist,fmt);
    vsprintf(__s,fmt,valist);
    va_end(valist);
    OLED_ShowString(x,y,(u8*)__s,size1,mode);
}
...
/*!
    ...
    \retval     none
*/
static void pixel_set(int16_t x, int16_t y, uint8_t t)
{
    u8 i, m, n;
    i = y / 8;
    m = y % 8;
...
{
    pixel_set(center_x+x, center_y+y,1);
    pixel_set(center_x-x, center_y+y,1);
    pixel_set(center_x+x, center_y-y,1);
    pixel_set(center_x-x, center_y-y,1);
}
...
void lcd_circle_draw(uint16_t xpos, uint16_t ypos, uint16_t radius)
{
    ...
    pixel_set(-radius+xpos, ypos,1);
    pixel_set(radius+xpos, ypos,1);
    pixel_set(xpos, -radius+ypos,1);
    pixel_set(xpos, radius+ypos,1);
    ...
    while(x <= y){
        ...
        pixel_set(x+xpos, y+ypos,1);
        pixel_set(-x+xpos, y+ypos,1);
        pixel_set(-x+xpos, -y+ypos,1);
        pixel_set(x+xpos, -y+ypos,1);
        pixel_set(y+xpos, x+ypos,1);
        pixel_set(-y+xpos, x+ypos,1);
        pixel_set(-y+xpos, -x+ypos,1);
        pixel_set(y+xpos, -x+ypos,1);
    }
