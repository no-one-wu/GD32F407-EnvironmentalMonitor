...
#include <rtdevice.h>
#include "ir.h"
static rt_base_t ir_pin = RT_NULL;
static rt_tick_t TICK_TIM = 0;
static void GPIO_IRQHandler(void *args)
{
    if(rt_pin_read(ir_pin)==PIN_LOW){
        TICK_TIM = rt_tick_get();
    }
}
void IR_Init(void)
{
    ir_pin = rt_pin_get("PC.8");  //输出
    rt_pin_mode(ir_pin, PIN_MODE_INPUT);
    /* 下降沿触发 */
    rt_pin_attach_irq(ir_pin, PIN_IRQ_MODE_FALLING, GPIO_IRQHandler, RT_NULL);
    rt_pin_irq_enable(ir_pin, PIN_IRQ_ENABLE);
}
//返回RT_TRUE有人，返回RT_FALSE没人
rt_bool_t get_ir_open_tim(){
    if(rt_pin_read(ir_pin)==PIN_LOW){
        TICK_TIM = rt_tick_get();
    }
